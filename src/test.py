...
def image_preprocessing(im,sess):
    ...
    im_tensor=tf.multiply(im_tensor,5)
...
def main(_):
    ...
    while ii<len(test_set):
        ...
        im=sess.run(ims)
        #print im==imm
        label=test_set[ii]['label_index']
        ii+=1
        class_cnt=[0 for i in range(6)]
        for logit in logits:   # for logit in logits:
            out=sess.run(logit,{is_training:False,images:im})
            index=0
            for i in range(6):
                if out[0][i]>out[0][index]:
                    index=i
            class_cnt[index]+=1
        if class_cnt.index(max(class_cnt))==label:
            correct_num+=1
        print class_cnt
        if label!=-1:
            total_num+=1
            print 1.0*correct_num/total_num
